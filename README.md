...
* **Automações de Conteúdo:** Varredura em tempo de execução via `preg_replace_callback` que injeta o atributo `alt` em imagens vazias (essencial para acessibilidade e SEO de imagens), além da injeção silenciosa de Schema Markup (JSON-LD) para geração de Rich Snippets.
